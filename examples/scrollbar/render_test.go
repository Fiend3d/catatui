...
func TestRender(t *testing.T) {
	verticalState := widgets.NewScrollbarState(100)
	horizontalState := widgets.NewScrollbarState(100)
	verticalState.Next()
	horizontalState.Next()
	...
	for _, size := range sizes {
		...
		if err := terminal.Draw(func(f *catatui.Frame) { render(f, &verticalState, &horizontalState) }); err != nil {
			t.Fatalf("%dx%d: %v", size[0], size[1], err)
		}
	}
}
